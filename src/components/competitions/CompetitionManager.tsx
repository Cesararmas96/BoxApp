...
interface CompetitionManagerProps {
    competition: any; // We'll refine this type
    onClose?: () => void;
}

export const CompetitionManager: React.FC<CompetitionManagerProps> = ({ competition }) => {
    const { t } = useLanguage();
        ...
        <DialogContent className="sm:max-w-[950px] max-h-[90vh] overflow-hidden p-0 gap-0 border-white/10 glass rounded-[3rem] shadow-2xl">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col h-[calc(90vh-160px)]">
                <div className="px-10 border-b bg-white/5 flex-shrink-0">
                    <TabsList className="h-16 bg-transparent gap-8 p-0 w-full justify-start overflow-x-auto scrollbar-none">
