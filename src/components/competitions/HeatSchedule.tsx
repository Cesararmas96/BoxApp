...
export const HeatSchedule: React.FC<HeatScheduleProps> = ({ heats, onHeatUpdate }) => {
    ...
    if (heats.length === 0) {
        return (
            <div className="col-span-full text-center p-12 bg-muted/50 rounded-[2rem] border border-dashed border-border flex flex-col items-center justify-center gap-4 opacity-50">
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-auto">
            {heats.map(heat => (
                <div key={heat.id} className="bg-muted/50 border border-border rounded-[2rem] p-6 space-y-6 hover:border-primary/20 transition-colors">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <Badge variant="outline" className="text-[10px] border-border text-muted-foreground">{heat.status}</Badge>
                            </div>
                        </div>
                            ...
                            <div className="space-y-2">
                                <div className="grid grid-cols-2 gap-2">
                                    {heat.lane_assignments?.sort((a, b) => a.lane_number - b.lane_number).map((assignment) => (
                                        <div key={assignment.participant_id} className="flex items-center gap-3 p-2 bg-muted/50 rounded-lg border border-border">
                                            <div className="h-6 w-6 rounded flex items-center justify-center bg-muted/50 text-[10px] font-black text-muted-foreground border border-border">
                                ...
                                {(!heat.lane_assignments || heat.lane_assignments.length === 0) && (
                                    <div className="text-center p-4 border border-dashed border-border rounded-xl hover:bg-muted transition-colors cursor-pointer group">
